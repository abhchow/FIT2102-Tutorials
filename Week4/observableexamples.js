...
function mousePosObservable() {
    const pos = document.getElementById("pos"), o = (0, rxjs_1.fromEvent)(document, "mousemove").pipe((0, operators_1.map)(({ clientX, clientY }) => ({ x: clientX, y: clientY })));
    o.pipe((0, operators_1.map)(({ x, y }) => `${x},${y}`)).subscribe((s) => (pos.innerHTML = s));
    o.pipe((0, operators_1.filter)(({ x }) => x > 400)).subscribe((_) => pos.classList.add("highlight"));
    o.pipe((0, operators_1.filter)(({ x }) => x <= 400)).subscribe(({ x, y }) => {
        pos.classList.remove("highlight");
    });
}
// Exercise 5
...
function piApproximation() {
    ...
    const randomNumberStream = (0, rxjs_1.interval)(50).pipe((0, operators_1.map)(nextRandom));
}
// Exercise 6
// ===========================================================================================
// ===========================================================================================
